...
class lenet(softmax):
    ...
    def __init__(self,_hyper,in_units=(28,28),out_units=3,ctx=mx.cpu()):
        self.hyper=_hyper
        self.ctx=ctx
        self.net,self.par  = self._init_net(in_units,out_units)
    ...
    def _init_net(self,in_units,out_units):
        net = gluon.nn.Sequential()#inicializacion api sequencial
        net.add(
            gluon.nn.Conv2D(channels=6, kernel_size=5, padding=2, activation='softsign'),
            gluon.nn.MaxPool2D(pool_size=2, strides=2),
            gluon.nn.Conv2D(channels=16, kernel_size=5, activation='softsign'),
            gluon.nn.MaxPool2D(pool_size=2, strides=2),
            ...
            gluon.nn.Dense(120, activation='softsign'), 
            gluon.nn.Dense(84, activation='softsign'))
